...
gene_clin_SARC <- subset(gene_clin, gene_clin$`cancer type abbreviation` == "SARC") #263 patients

#Subset info file to obtain DSS and PFI 
info_surv_DSS <- gene_clin_SARC[,c(1,2,3,28,29)]
info_surv_PFI <- gene_clin_SARC[,c(1,2,3,32,33)]
...
info_surv_DSS$TRIM6_expression <- ifelse(info_surv_DSS$TRIM6 >= 5.666906, 'High', "Low")
info_surv_PFI$TRIM6_expression <- ifelse(info_surv_PFI$TRIM6 >= 5.666906, 'High', "Low")

table(info_surv_DSS$TRIM6_expression) #high vs low 
table(info_surv_PFI$TRIM6_expression) #high vs low 


#### for DSS #### 
#Remove timepoints that start at 0 
info_surv_DSS <- subset(info_surv_DSS, info_surv_DSS$DSS.time > 0) #before:263, after: 263

#Convert DSS.time into years 
info_surv_DSS$years <- info_surv_DSS$DSS.time/365

#remove NAs
#before removing: 263, after: 257
info_surv_DSS <- info_surv_DSS[complete.cases(info_surv_DSS),]

#Define survival
survival = Surv(time= info_surv_DSS$years, event = info_surv_DSS$DSS)

survival_fit<- survfit(formula = survival ~ info_surv_DSS$TRIM6_expression, data = info_surv_DSS)

table(info_surv_DSS$cancer_type) #1, the one we are interested in 
table(info_surv_DSS$TRIM6_expression) #high vs low 


dss_t6 <- ggsurvplot(fit= survival_fit, 
           pval = TRUE, 
           surv.median.line = "hv", legend = c(0.1,0.2),
           xlab = "DSS (Years)", 
           ylab = "DSS Probability",
           title = "TRIM6 DSS for SARC",
           ylim=c(0.0,1), 
           xlim=c(0,16),
...
pdf("dss_TRIM6.pdf", width = 8, height = 8, onefile = F)
print(dss_t6)
dev.off()

### TRIM6 PFI ###
#Remove timepoints that start at 0 for PFI
info_surv_PFI <- subset(info_surv_PFI, info_surv_PFI$PFI.time > 0) #before: 263, after:263

#Convert PFI.time into years 
info_surv_PFI$years <- info_surv_PFI$PFI.time/365

#remove NAs
#before removing: 263, after:263
...
survival_fit<- survfit(formula = survival ~ info_surv_PFI$TRIM6_expression, data = info_surv_PFI)

pfi_t6 <- ggsurvplot(fit= survival_fit, 
           pval = TRUE, 
           surv.median.line = "hv", legend = c(0.07,0.2),
           xlab = "PFI (Years)", 
           ylab = "PFI Probability",
           title = "TRIM6 PFI for SARC",
           ylim=c(0.0,1), 
           xlim=c(0,15),
           palette = c("#E69F00","#0072B2"),
...
pdf("pfi_TRIM6.pdf", width = 8, height = 8, onefile = F)
print(pfi_t6)
dev.off()