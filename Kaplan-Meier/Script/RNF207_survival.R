...
#Merge the genes file with clinical data (for rnf207 in this case)
gene_clin <- merge(genes, clin_tcga, by = "sample")

#Subset cancer type of interest (the cancer type that had highest variance)
gene_clin_GBM <- subset(gene_clin, gene_clin$`cancer type abbreviation` == "GBM") #173 patients

#Subset info file to obtain DSS and PFI 
info_surv_DSS <- gene_clin_GBM[,c(1,2,3,28,29)]
info_surv_PFI <- gene_clin_GBM[,c(1,2,3,32,33)]
...
#Define TRIM6 expression based on high/low
info_surv_DSS$RNF207_expression <- ifelse(info_surv_DSS$RNF207 >= 4.530978, 'High', "Low")
info_surv_PFI$RNF207_expression <- ifelse(info_surv_PFI$RNF207 >= 4.530978, 'High', "Low")

table(info_surv_DSS$RNF207_expression) #high vs low
table(info_surv_PFI$RNF207_expression) #high vs low


#### for DSS #### 
#Remove timepoints that start at 0 
info_surv_DSS <- subset(info_surv_DSS, info_surv_DSS$DSS.time > 0) #before: 173, after:173

#Convert DSS.time into years 
info_surv_DSS$years <- info_surv_DSS$DSS.time/365

#remove NAs
#before removing: 173, after: 160
info_surv_DSS <- info_surv_DSS[complete.cases(info_surv_DSS),]

#Define survival
survival = Surv(time= info_surv_DSS$years, event = info_surv_DSS$DSS)

survival_fit<- survfit(formula = survival ~ info_surv_DSS$RNF207_expression, data = info_surv_DSS)

table(info_surv_DSS$cancer_type) #1, the one we are interested in 
table(info_surv_DSS$RNF207_expression) #high vs low 


dss_rnf207 <- ggsurvplot(fit= survival_fit, 
           pval = TRUE, 
           surv.median.line = "hv", legend = c(0.1,0.2),
           xlab = "DSS (Years)", 
           ylab = "DSS Probability",
           title = "RNF207 DSS for GBM",
           ylim=c(0.0,1), 
           xlim=c(0,7.3),
...
pdf("dss_rnf207.pdf", width = 8, height = 8, onefile = F)
print(dss_rnf207)
dev.off()

### RNF207 PFI ###
#Remove timepoints that start at 0 for PFI
info_surv_PFI <- subset(info_surv_PFI, info_surv_PFI$PFI.time > 0) #before: 173, after:173

#Convert PFI.time into years 
info_surv_PFI$years <- info_surv_PFI$PFI.time/365

#remove NAs
#before removing: 173, after: 173
info_surv_PFI <- info_surv_PFI[complete.cases(info_surv_PFI),]

#Define survival 
survival = Surv(time= info_surv_PFI$years, event = info_surv_PFI$PFI)

survival_fit<- survfit(formula = survival ~ info_surv_PFI$RNF207_expression, data = info_surv_PFI)

pfi_rnf207 <- ggsurvplot(fit= survival_fit, 
           pval = TRUE, 
           surv.median.line = "hv", legend = c(0.07,0.2),
           xlab = "PFI (Years)", 
           ylab = "PFI Probability",
           title = "RNF207 PFI for GBM",
           ylim=c(0.0,1), 
           xlim=c(0,5),
           palette = c("#E69F00","#0072B2"),
           pval.coord=c(-0.2,0.05), 
           break.x.by= 1,         
           conf.int = T,
           risk.table = T, risk.table.title="",
           risk.table.height = 0.15,
...
pdf("pfi_rnf207.pdf", width = 8, height = 8, onefile = F)
print(pfi_rnf207)
dev.off()
