...
gene_clin_CESC <- subset(gene_clin, gene_clin$`cancer type abbreviation` == "CESC") #306 patients

#Subset info file to obtain DSS and PFI 
info_surv_DSS <- gene_clin_CESC[,c(1,2,3,28,29)]
info_surv_PFI <- gene_clin_CESC[,c(1,2,3,32,33)]
...
#Define TRIM2 expression based on high/low
info_surv_DSS$TRIM2_expression <- ifelse(info_surv_DSS$TRIM2 >= 8.602529, 'High', "Low")
info_surv_PFI$TRIM2_expression <- ifelse(info_surv_PFI$TRIM2 >= 8.602529, 'High', "Low")

table(info_surv_DSS$TRIM2_expression) #high vs low
table(info_surv_PFI$TRIM2_expression) #high vs low

#### for DSS #### 
#Remove timepoints that start at 0 
info_surv_DSS <- subset(info_surv_DSS, info_surv_DSS$DSS.time > 0) #before:306, after:293

#Convert DSS.time into years 
info_surv_DSS$years <- info_surv_DSS$DSS.time/365

#remove NAs
#before removing: 293, after: 289
info_surv_DSS <- info_surv_DSS[complete.cases(info_surv_DSS),]

#Define survival
survival = Surv(time= info_surv_DSS$years, event = info_surv_DSS$DSS)

survival_fit<- survfit(formula = survival ~ info_surv_DSS$TRIM2_expression, data = info_surv_DSS)

table(info_surv_DSS$cancer_type) #1, the one we are interested in 
table(info_surv_DSS$TRIM2_expression) #high vs low 

dss_t2 <- ggsurvplot(fit= survival_fit, 
           pval = TRUE, 
           surv.median.line = "hv", legend = c(0.1,0.2),
           xlab = "DSS (Years)", 
           ylab = "DSS Probability",
           title = "TRIM2 DSS for CESC",
           ylim=c(0.0,1), 
           xlim=c(0,18),
...
pdf("dss_trim2.pdf", width = 8, height = 8, onefile = F)
print(dss_t2)
dev.off()
...
info_surv_PFI <- subset(info_surv_PFI, info_surv_PFI$PFI.time > 0) #before: 306, after: 293

#Convert PFI.time into years 
info_surv_PFI$years <- info_surv_PFI$PFI.time/365

#remove NAs
#before removing: 293, after: 293
info_surv_PFI <- info_surv_PFI[complete.cases(info_surv_PFI),]

#Define survival 
survival = Surv(time= info_surv_PFI$years, event = info_surv_PFI$PFI)

survival_fit<- survfit(formula = survival ~ info_surv_PFI$TRIM2_expression, data = info_surv_PFI)

pfi_t2 <- ggsurvplot(fit= survival_fit, 
           pval = TRUE, 
           surv.median.line = "hv", legend = c(0.1,0.2),
           xlab = "PFI (Years)", 
           ylab = "PFI Probability",
           title = "TRIM2 PFI for CESC",
           ylim=c(0.0,1), 
           xlim=c(0,18),
           palette = c("#E69F00","#0072B2"),
           pval.coord=c(-0.06,0.05), 
...
pdf("pfi_trim2.pdf", width = 8, height = 8, onefile = F)
print(pfi_t2)
dev.off()
